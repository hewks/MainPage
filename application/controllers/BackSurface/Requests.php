...
class Requests extends CI_Controller
{
    ...
    function index()
    {
        ...
        $footer_data = array(
            ...
            <script src="' . base_url() . 'assets/js/BackSurface/components/pages/requestsTableSection.js"></script>'
        );

        $section_data = array(
            'page_title' => 'Peticiones'
        );

        $this->load->view('BackSurface/layout/header', $header_data);
        $this->load->view('BackSurface/layout/navigation');
        $this->load->view('BackSurface/pages/requests', $section_data);
    ...
    function data_table()
    {
        ...
        switch ($this->input->post('requestType')) {
            case 'all':
                $all_data = $this->Model_Request->all('table');
                ...
                break;
            case 'one':
                $one_data = $this->Model_Request->one($this->input->post('id'));
    ...
    function add()
    {
        ...
        if (!$this->genetic->validate_data($new_data)) {
            ...
            if (!$this->Model_Request->create($new_data)) {
                $output[] = array(
                    'status' => false,
    ...
    function delete()
    {
        ...
        if (!$this->Model_Request->delete($this->input->post('id'))) {
            $output[] = array(
                'status' => false,
    ...
    function active()
    {
        ...
        if (!$this->Model_Request->active($this->input->post('id'))) {
            $output[] = array(
                'status' => false,
